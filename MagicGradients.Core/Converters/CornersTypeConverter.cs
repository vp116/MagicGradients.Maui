...
public class CornersTypeConverter : TypeConverter
{
    ...
    public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
    {
        ...
        if (dim.Length == 1) return new Corners(new Dimensions(Offset.Parse(dim[0], OffsetType.Absolute)));
        ...
        Type destinationType)
    {
        if (value is Corners c)
            return
                $"{c.TopLeft.Width.ToStringWithUnit()} {c.TopRight.Width.ToStringWithUnit()} {c.BottomRight.Width.ToStringWithUnit()} {c.BottomLeft.Width.ToStringWithUnit()}";
