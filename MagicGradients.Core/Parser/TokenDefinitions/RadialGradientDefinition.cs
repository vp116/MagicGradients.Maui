...
public class RadialGradientDefinition : ITokenDefinition
{
    ...
    {
        return token == CssToken.RadialGradient ||
    ...
    public void Parse(CssReader reader, GradientBuilder builder)
    {
        ...
        if (!hasShape && !hasStretch && !hasRadius && !hasPos) reader.Rollback();
    }
    ...
    private (bool, Position) GetPosition(CssReader reader)
    {
        if (reader.CanRead)
        {
            ...
            if (token == "at")
            {
                ...
                if (!isPosX && !string.IsNullOrEmpty(tokenX)) direction.SetNamedDirection(tokenX);

                if (!isPosY && !string.IsNullOrEmpty(tokenY)) direction.SetNamedDirection(tokenY);
