...
public class RadialGradientGeometry : GradientGeometry<IRadialGradient>
{
    ...
    private SizeF GetRadius(IRadialGradient gradient, PointF center, RectF rect, float offset, float pixelScaling)
    {
        ...
        if (gradient.Radius.Width.Value > 0) radiusX = gradient.Radius.Width.GetDrawPixels(rect.Width, pixelScaling);

        if (gradient.Radius.Height.Value > 0) radiusY = gradient.Radius.Height.GetDrawPixels(rect.Height, pixelScaling);
    ...
    {
        var points = gradient.Stretch.IsCorner() ? GetCornerPoints(rect) : GetSidePoints(center, rect);

        var distances = new PointF[points.Length];

        for (var i = 0; i < distances.Length; i++) distances[i] = Subtract(center, points[i]);
    ...
    private float[] GetEuclideanDistance(IRadialGradient gradient, PointF center, RectF rect)
    {
        ...
        for (var i = 0; i < distances.Length; i++) distances[i] = center.Distance(points[i]);
