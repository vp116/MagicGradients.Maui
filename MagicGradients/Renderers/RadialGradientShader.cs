...
public class RadialGradientShader : IGradientShader
{
    ...
    {
        var points = _gradient.Size.IsCorner() ? GetCornerPoints(rect) : GetSidePoints(center, rect);

        var distances = new SKPoint[points.Length];

        for (var i = 0; i < distances.Length; i++) distances[i] = center - points[i];
    ...
    private float[] GetEuclideanDistance(SKPoint center, SKRect rect)
    {
        ...
        for (var i = 0; i < distances.Length; i++) distances[i] = SKPoint.Distance(center, points[i]);
    ...
    {
        if (radiusX > radiusY) return SKMatrix.MakeScale(radiusX / radiusY, 1f, center.X, center.Y);

        if (radiusY > radiusX) return SKMatrix.MakeScale(1f, radiusY / radiusX, center.X, center.Y);
