...
public class GradientRenderer<T> where T : VisualElement, IGradientControl
{
    ...
    private void RenderTiled(RenderContext context)
    {
        ...
        var rows = _control.GradientRepeat == Repeat || _control.GradientRepeat == RepeatY
            ? (int)Math.Ceiling((double)height / tileHeight)
            : 1;

        var cols = _control.GradientRepeat == Repeat || _control.GradientRepeat == RepeatX
            ? (int)Math.Ceiling((double)width / tileWidth)
            : 1;
    ...
    {
        var gradient = new GradientBuilder()
            .AddCssGradient("linear-gradient(43deg, #4158D0 0%, #C850C0 46%, #FFCC70 100%)").Build().First();
        gradient.InstantiateShader(_control);
        gradient.Measure(context.RenderRect.Width, context.RenderRect.Height);
