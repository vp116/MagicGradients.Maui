...
{
    public virtual StopsFactory StopsFactory { get; } = new();
    ...
    {
        foreach (var offset in offsets) StopsFactory.CreateStop(color, offset);
        return Instance;
    }

    public T AddStops(params Color[] colors)
    {
        foreach (var color in colors) StopsFactory.CreateStop(color);
        return Instance;
    }
}